...
const SnaggingDetail = dynamic(() => import('@/components/snagging/SnaggingDetail').then(mod => ({ default: mod.SnaggingDetail })), {
  loading: () => (
...
export default function SnaggingDetailPage({
  ...
  const { data: snagging, isLoading, error, mutate } = useSWR<Snagging>(
    id ? `/snaggings/${id}` : null,
    () => snaggingService.getSnaggingById(id),
    {
