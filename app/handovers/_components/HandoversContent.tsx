...
import { getHandoverStats, HandoverStats } from '@/lib/api/handover.service'
import { HandoverFilters, HandoverStatus, Handover } from '@/lib/types/handover.types'
import { Plus, Search, RefreshCw, FileText, Clock, CheckCircle, AlertCircle, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function HandoversContent() {
  ...
  const { data: stats } = useSWR<HandoverStats>(
    isAdmin ? '/handovers/stats' : null,
    () => getHandoverStats(),
  ...
  const totalHandovers = isAdmin ? (stats?.total ?? pagination?.total ?? 0) : (pagination?.total ?? 0)
  const pendingAction = isAdmin
    ? (stats?.pendingAction ?? 0)
    : handovers.filter(h => h.status === HandoverStatus.SENT_TO_OWNER).length
  const drafts = isAdmin
    ? (stats?.draft ?? 0)
    : handovers.filter(h => h.status === HandoverStatus.DRAFT).length
  const accepted = isAdmin
    ? (stats?.accepted ?? 0)
    : handovers.filter(h => h.status === HandoverStatus.ACCEPTED).length
